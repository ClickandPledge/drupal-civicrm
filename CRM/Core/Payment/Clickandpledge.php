...
class CRM_Core_Payment_Clickandpledge extends CRM_Core_Payment {
  ...
  function doDirectPayment(&$params) {
    ...
    if (!in_array($params['currencyID'], $allowed_currencies)){
      return self::errorExit(9099, 'It appears that selected currency is not supported with this payment processor.
        If you continue to have problems please contact the site administrator.');
    }
    ...
    if (isset($params['is_recurr']) && $params['is_recur'] == TRUE) {
      ...
      if (isset($params['installments']) && $params['installments'] != '') {
        if ($params['installments'] <= 1 || $params['installments'] >= 1000) {
      ...
      $params['billing_middle_name'] = substr($params['billing_middle_name'], 0, 1);
    ...
    if(trim($item_parts[0]) == 'Online Event Registration' ) {
      ...
      */
      ...
      $query = "SELECT civicrm_event.id as event_id,civicrm_event.title as title, civicrm_financial_type.is_deductible,
        civicrm_financial_type.name,civicrm_event.is_email_confirm,civicrm_event.confirm_email_text
        FROM civicrm_event INNER JOIN civicrm_financial_type ON civicrm_event.financial_type_id = civicrm_financial_type.id
  ...
  function buildXML( $params, $xmlpath )
  {
	  $configValues = $this->_paymentProcessor;
		...
		$dom = new DOMDocument('1.0', 'UTF-8');
			 ...
			 $applicationid=$dom->createElement('ID','CnP_CiviCRM_Drupal'); //
			 $applicationid=$application->appendChild($applicationid);

			 $applicationname=$dom->createElement('Name','CnP_CiviCRM_Drupal'); //CnP_CiviCRM_WordPress#CnP_CiviCRM_Joomla#CnP_CiviCRM_Drupal
     ...
     // die();
            ...
            $sql_params = array(0 => array($params['pcp_made_through_id'], 'Integer'));
						$dao = CRM_Core_DAO::executeQuery($query, $sql_params);
						if ($dao->fetch()) {
            ...
            $sql_params = array(0 => array($params['contributionPageID'], 'Integer'));
                ...
                $sql_params = array(0 => array($dao->campaign_id, 'Integer'));
								$dao_campaign = CRM_Core_DAO::executeQuery($query_campaign, $sql_params);
					...
					elseif((isset( $params['campaign_id'] ) && $params['campaign_id'] != '') && (!isset( $params['contributionPageID'] )))
					{
						...
						$query_campaign = "SELECT *  FROM  civicrm_campaign  WHERE  id = %0";
            $sql_params = array(0 => array($params['campaign_id'], 'Integer'));
						$dao_campaign = CRM_Core_DAO::executeQuery($query_campaign, $sql_params);
						if ($dao_campaign->fetch())
						{
        ...
        */
        ...
        $query = "SELECT *  FROM  civicrm_financial_type  WHERE  id = %0";
          $sql_params = array(0 => array($params['contributionTypeID'], 'Integer'));
					$dao = CRM_Core_DAO::executeQuery($query, $sql_params);
					if ($dao->fetch()) {
        ...
        */
        ...
        $query = "SELECT *  FROM  civicrm_financial_type  WHERE  name = %0";
          $sql_params = array(0 => array($params['contributionType_name'], 'String'));
					$dao = CRM_Core_DAO::executeQuery($query, $sql_params);
					if ($dao->fetch()) {
						if( $dao->is_deductible == '1' )
						{
          ...
          */
          ...
          $sql_params = array(0 => array($params['selectProduct'], 'Integer'));
					$dao = CRM_Core_DAO::executeQuery($query, $sql_params);
					if ($dao->fetch()) {
